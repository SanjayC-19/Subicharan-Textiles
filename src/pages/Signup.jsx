    ...
    <section className="min-h-screen pt-10 pb-16 px-4 sm:px-6 lg:px-8 bg-background">
      <div className="max-w-5xl mx-auto bg-card border border-border grid md:grid-cols-2 overflow-hidden">
        <div className="p-6 sm:p-8 md:p-10">
          ...
          <h1 className="mt-2 text-4xl font-serif text-foreground">Sign Up</h1>
          <p className="mt-2 font-sans text-sm text-muted-foreground">Create your account in a few steps.</p>

          <form className="mt-8 space-y-4" onSubmit={handleSubmit} noValidate>

            <div>
              <label htmlFor="name" className="block mb-2 font-sans text-xs tracking-[0.1em] uppercase text-foreground/80">
                Full Name
              </label>
              <input
                id="name"
              ...
              />
              {errors.name ? <p className="mt-1 text-xs text-destructive font-sans">{errors.name}</p> : null}
            </div>
            ...
            <div>
              <label htmlFor="role" className="block mb-2 font-sans text-xs tracking-[0.1em] uppercase text-foreground/80">
                Role
              </label>
              <select
                id="role"
              ...
              {errors.role ? <p className="mt-1 text-xs text-destructive font-sans">{errors.role}</p> : null}
            </div>
        ...
        <div className="relative hidden md:block min-h-[520px]">
          ...
          <div className="absolute bottom-8 left-8 right-8">
            ...
            <p className="mt-2 font-serif text-3xl text-primary-foreground">Built around craft, quality and confidence.</p>
          </div>
        </div>
      </div>
    </section>
